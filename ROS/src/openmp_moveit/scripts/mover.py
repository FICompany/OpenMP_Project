...
from openmp_moveit.srv import MoverService, MoverServiceRequest, MoverServiceResponse
...
def plan_pick_and_place(req):
    ...
    pre_pose = copy.deepcopy(req.pick_pose)
    pre_pose.position.z += 0.1   # Static value coming from Unity, TODO: pass along with request

    pre_grasp_pose = plan_trajectory(move_group, pre_pose, current_robot_joint_configuration)
    ...
    pick_up_pose = plan_trajectory(move_group, pre_pose, previous_ending_joint_angles)
    ...
    place = copy.deepcopy(req.place_pose)
    place.position.z += 0.08
    
    place_pose = plan_trajectory(move_group, place, previous_ending_joint_angles)
...
def moveit_server():
    ...
    rospy.init_node('openmp_moveit_server')

    s = rospy.Service('openmp_moveit', MoverService, plan_pick_and_place)
    print("Ready to plan")
    rospy.spin()
